...
# * the maisonverte droplet should be deleted in "Post-build Actions".
...
IP=$(ssh "$DOCKERHOSTUSER@$DOCKERHOST" \
  "./digitalocean/scripts/new-droplet.sh maisonverte")
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no \
  root@"$IP" \
  "git clone https://github.com/maisonverte/maisonverte.git && \
  cd site-web && \
  ./scripts/ci.sh"
